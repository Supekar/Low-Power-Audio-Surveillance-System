...
double arduinoFFT::MajorPeak(double *vD, uint16_t samples, double samplingFrequency)
{
  ...
  uint16_t IndexOfMaxY = 0;
  double maxY2 = 0;
  uint16_t IndexOfMaxY2 = 0;

  
  //If sampling_frequency = 2 * max_frequency in signal,
  //value would be stored at position samples/2
  for (uint16_t i = 1; i < ((samples >> 1) + 1); i++) {
 //SerialUSB.print(vD[i]);  
 //SerialUSB.print("\t");  
  //SerialUSB.println((i*samplingFrequency)/(samples-1));
    if ((vD[i-1] < vD[i]) && (vD[i] > vD[i+1])) {
      if (vD[i] > maxY) {
        maxY2 = maxY;
        maxY = vD[i];
        ...
        IndexOfMaxY = i;
       // SerialUSB.println((IndexOfMaxY*samplingFrequency)/(samples-1));
       // SerialUSB.println((IndexOfMaxY2*samplingFrequency)/(samples-1));
      }

     else if (vD[i] > maxY2 && vD[i] != maxY) 
     {
            maxY2 = vD[i];
            IndexOfMaxY2 = i; 
     }
 ...
 //SerialUSB.println(maxY);
 //SerialUSB.print("IndexOfMaxY is:");
 //SerialUSB.println(IndexOfMaxY);

 
 //SerialUSB.print("maxY2 is:");
 //SerialUSB.println(maxY2);
 //SerialUSB.print("IndexOfMaxY2 is:");
 //SerialUSB.println(IndexOfMaxY2);
...
// SerialUSB.print("Highest peak:");
// SerialUSB.println(interpolatedX);
...
// SerialUSB.println(delta);
  ...
  double delta2 = 0.5 * ((vD[IndexOfMaxY2-1] - vD[IndexOfMaxY2+1]) / (vD[IndexOfMaxY2-1] - (2.0 * vD[IndexOfMaxY2]) + vD[IndexOfMaxY2+1]));
   interpolatedX2 = ((IndexOfMaxY2 + delta2)  * samplingFrequency) / (samples-1);
  if(IndexOfMaxY2==(samples >> 1)) //To improve calculation on edge values
    interpolatedX2 = ((IndexOfMaxY2 + delta2)  * samplingFrequency) / (samples);
  // returned value: interpolated frequency peak apex

//  SerialUSB.print("Second highest peak:");
// SerialUSB.println(interpolatedX2);
 //SerialUSB.println((IndexOfMaxY2*samplingFrequency)/(samples-1));
 // SerialUSB.print("Delta2:");
 // SerialUSB.println(delta2);
...
double arduinoFFT::SecondMajorPeak()
{
	
  return(interpolatedX2);
//	
//	double secondMaxY = 0;
//	
//	uint16_t IndexOfSecondMaxY = 0;
// 
//	//If sampling_frequency = 2 * max_frequency in signal,
//	//value would be stored at position samples/2
//	for (uint16_t i = 1; i < ((samples >> 1) + 1); i++) {
...
//        secondMaxY = maxY;
//        maxY = vD[i];
//        IndexOfSecondMaxY = IndexOfMaxY;
//        IndexOfMaxY = i;
//			}
//		}
//
//      else if (vD[i] > secondMaxY && vD[i] != maxY) 
//     {
//            secondMaxY = vD[i];
//            IndexOfSecondMaxY = i; 
//     }
...
//
//
//	double delta = 0.5 * ((vD[IndexOfSecondMaxY - 1] - vD[IndexOfSecondMaxY + 1]) / (vD[IndexOfSecondMaxY - 1] - (2.0 * vD[IndexOfSecondMaxY]) + vD[IndexOfSecondMaxY + 1]));
//	double interpolatedX = ((IndexOfSecondMaxY + delta)  * samplingFrequency) / (samples - 1);
//	if (IndexOfSecondMaxY == (samples >> 1)) //To improve calculation on edge values
//		interpolatedX = ((IndexOfSecondMaxY + delta)  * samplingFrequency) / (samples);
//	// returned value: interpolated frequency peak apex
//	return(interpolatedX);
}
